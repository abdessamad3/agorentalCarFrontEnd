...
export class VidangeListComponent implements OnInit {
  ...
  constructor(private crud: CrudService, private ts: TranslationService, private fb: FormBuilder, private bus: EventBusService) {
    this.form = this.fb.group({
      kilometrageSuivant: [0, [Validators.required, Validators.min(0)]],
      filtreAir:          [false],
      filtreHuile:        [false],
  ...
  openAdd()             { this.selected = null; this.isEditing = false; this.form.reset({ kilometrageSuivant: 0, filtreAir: false, filtreHuile: false, filtreCarburant: false }); this.modalMode = 'form'; }

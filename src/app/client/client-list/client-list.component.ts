...
export class ClientListComponent implements OnInit {
  ...
  get filtered() {
    let list = this.items;
    if (this.filterNationalite) list = list.filter(i => i.nationalite === this.filterNationalite);
    if (!this.search.trim()) return list;
    const q = this.search.toLowerCase();
    return list.filter(i => Object.values(i).some(v => String(v).toLowerCase().includes(q)));
  }
